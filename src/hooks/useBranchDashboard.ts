...
export function useBranchDashboard() {
  ...
  return useQuery({
    queryKey: ['branch-dashboard', currentOrganization?.id],
    queryFn: async () => {
      ...
      const branchPerformance: BranchPerformance[] = (branches || []).map(branch => {
        ...
        const branchVisits = visitsToday?.filter(v => branchAgentIds.includes(v.user_id)) || [];
        const branchAttendance = attendanceToday?.filter(a => branchAgentIds.includes(a.user_id)) || [];
        const branchPlans = plansToday?.filter(p => branchAgentIds.includes(p.user_id)) || [];
      ...
      const totalVisits = visitsToday?.length || 0;
      const overallAttendance = totalAgents > 0 ? Math.round((totalActiveAgents / totalAgents) * 100) : 0;

      const totalOrgTarget = plansToday?.reduce((s, p) => s + (p.prospects_target || 0) + (p.quotes_target || 0) + (p.policies_target || 0), 0) || 0;
      const totalOrgActual = plansToday?.reduce((s, p) => s + (p.prospects_actual || 0) + (p.quotes_actual || 0) + (p.policies_actual || 0), 0) || 0;
